...
import { YEARTABLE } from "../constants/yearConstants"
import { YearApiData } from "../contextApi/year/yearContextApi"

const YearTable = () => {
  const { yearList, searchRecord } = useContext(YearApiData)
    ...
    <>
      <table className="w-full table-auto rounded">
        ...
        >
          ...
          {yearList.map((item) => (
            <tr key={item.id} className="border-t border-gray-200">
              <td className="border border-gray-200 py-4 px-2">{item.year}</td>
